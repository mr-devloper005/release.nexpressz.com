  ...
  return (
    <EditableSiteShell>
      <main className="mx-auto max-w-[1100px] px-4 py-10 sm:px-6 lg:px-8">
        <section className="rounded-[2rem] border border-[var(--slot4-line)] bg-white p-6 shadow-[0_18px_40px_rgba(107,116,69,0.10)] sm:p-8">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <p className="inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.24em] text-[var(--slot4-soft-muted-text)]">
                <MessageSquare className="h-4 w-4" /> Local comments
              </p>
              <h1 className="mt-4 text-4xl font-black tracking-[-0.04em] text-[var(--slot4-page-text)] sm:text-5xl">Comments</h1>
            ...
            <button type="button" className="rounded-full border border-[var(--slot4-line)] px-4 py-2 text-sm font-bold text-[var(--slot4-page-text)]" onClick={refreshComments}>Refresh comments</button>
          </div>
          ...
          <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="relative w-full sm:max-w-md">
              ...
              <input
                ...
                className="h-11 w-full rounded-[1.3rem] border border-[var(--slot4-line)] bg-[var(--slot4-warm)] pl-9 pr-3 text-sm outline-none"
              />
            </div>
        ...
        {visibleComments.length ? (
          <section className="mt-8 grid gap-4">
            {visibleComments.map((item) => (
              <article key={`${item.articleSlug}-${item.id}`} className="rounded-[1.6rem] border border-[var(--slot4-line)] bg-white p-5 shadow-[0_12px_30px_rgba(107,116,69,0.08)]">
                <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                  ...
                  {item.articleSlug ? (
                    <Link href={`/article/${item.articleSlug}`} className="text-sm font-semibold text-[var(--slot4-accent-fill)] underline-offset-4 hover:underline">
                      Open article
                    </Link>
                  ) : null}
                </div>
          ...
          <section className="mt-8 rounded-[1.8rem] border border-dashed border-[var(--slot4-line)] bg-white p-8 text-center">
        ...
        {filtered.length > COMMENTS_PER_PAGE ? (
          <div className="mt-8 flex flex-wrap items-center justify-between gap-3 rounded-[1.8rem] border border-[var(--slot4-line)] bg-white p-4 text-sm text-[var(--slot4-muted-text)] shadow-[0_12px_30px_rgba(107,116,69,0.08)]">
            <span>Page {currentPage} of {totalPages}</span>
            <div className="flex gap-2">
              <button type="button" className="rounded-full border border-[var(--slot4-line)] px-4 py-2 font-bold text-[var(--slot4-page-text)] disabled:opacity-40" disabled={currentPage <= 1} onClick={() => setPage((value) => Math.max(1, value - 1))}>Previous</button>
              <button type="button" className="rounded-full border border-[var(--slot4-line)] px-4 py-2 font-bold text-[var(--slot4-page-text)] disabled:opacity-40" disabled={currentPage >= totalPages} onClick={() => setPage((value) => Math.min(totalPages, value + 1))}>Next</button>
            </div>
          </div>
        ) : null}
      </main>
